...
export function Index({ state, posts }: IndexProps) {
  ...
  return (
    <div class="home" style="background-image:url('https://dash.deno.com/assets/background-pattern.svg')">
      {state.header || (
        <header
      ...
      <div class="max-w-screen-sm px-6 mx-auto">
        <div class="pt-10 lt-sm:pt-12 border-t-1 border-gray-300/80">
          {postIndex.map((post) => (
            <PostCard
              post={post}
              key={post.pathname}
...
export function PostPage({ post, state }: PostPageProps) {
  ...
  return (
    <div className={`post ${post.pathname.substring(1)}`} style="background-image:url('https://dash.deno.com/assets/background-pattern.svg')">
...
function Footer(props: { author?: string }) {
  return (
    <footer class="mt-20 pb-16 lt-sm:pb-8 lt-sm:mt-16" style="background-image:url('https://dash.deno.com/assets/background-pattern.svg')">
      <p class="flex items-center gap-2.5 text-gray-400/800 dark:text-gray-500/800 text-sm">
        <span>
          &copy; {new Date().getFullYear()} <a class="inline-flex items-center gap-1 underline hover:text-gray-800 dark:hover:text-gray-200 transition-colors" href="https://github.com/luanlops">
            
            {props.author} </a> &middot; Powered by
          {" "}
          <a
